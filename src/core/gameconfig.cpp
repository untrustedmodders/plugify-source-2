...
void* CGameConfig::GetAddress(const std::string& name) const {
	...
	void* addr = ResolveSignature(addrConf.signature);
	if (!addr)
		return nullptr;
	...
	for (size_t i = 0; i < readCount; ++i) {
		...
		if (addr == nullptr || addr < VALID_MINIMUM_MEMORY_ADDRESS)
			return nullptr;
...
CGameConfig* CGameConfigManager::LoadGameConfigFile(const char* file) {
	std::string path = utils::GamedataDirectory() + file;
