...
struct netadr_t_s2
{
	union {
		...
		// IPv4
		//
		uint32_t			m_unIPv4;			// IP stored in host order (little-endian)
		struct
		{
		...
		uint64_t	m_ipv6Qword[2];	// In a few places we can use these to avoid memcmp. BIG ENDIAN!
	};

	uint32_t		m_unIPv6Scope;			// IPv6 scope
