...
namespace globals
{
	...
	void Initialize()
	{
		...
		g_pGameResourceServiceServer = FindInterface<IGameResourceServiceServer>(modules::server, GAMERESOURCESERVICESERVER_INTERFACE_VERSION);
		...
		g_pEngineSound = FindInterface<IEngineSound>(modules::engine, IENGINESOUND_CLIENT_INTERFACE_VERSION);
