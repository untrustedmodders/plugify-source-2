...
{
    return dynamic_cast<CBaseEntity*>(g_pGameEntitySystem->GetEntityInstance(CEntityIndex(entityIndex)));
}
...
{
    CBaseEntity* pEntity = dynamic_cast<CBaseEntity*>(g_pGameEntitySystem->GetEntityInstance(CEntityIndex(entityIndex)));
    if (!pEntity)
    {
        return INVALID_EHANDLE_INDEX;
    }
...
extern "C" PLUGIN_API int EntHandleToEntIndex(int entityHandle)
{
    ...
    CBaseEntity* pEntity = dynamic_cast<CBaseEntity*>(g_pGameEntitySystem->GetEntityInstance(handle));
    if (!pEntity)
    {
        return -1;
    }
...
{
    CBaseEntity* pStartStart = dynamic_cast<CBaseEntity*>(g_pGameEntitySystem->GetEntityInstance(CEntityHandle(startEntity)));
    /*if (!pStartStart)
    {
        return INVALID_EHANDLE_INDEX;
    }*/

    CBaseEntity* pEntity = dynamic_cast<CBaseEntity*>(addresses::CGameEntitySystem_FindEntityByClassName(g_pGameEntitySystem, pStartStart, classname.c_str()));
    if (!pEntity)
    {
...
extern "C" PLUGIN_API int FindEntityByName(int startEntity, const plg::string& name)
{
    ...
    CBaseEntity* pEntity = dynamic_cast<CBaseEntity*>(addresses::CGameEntitySystem_FindEntityByName(g_pGameEntitySystem, pStartStart, name.c_str(), nullptr, nullptr, nullptr, nullptr));
    if (!pEntity)
    {
...
{
    CBaseEntity* pEntity = dynamic_cast<CBaseEntity*>(addresses::CreateEntityByName(className.c_str(), -1));
    if (!pEntity)
    {
...
{
	CBaseModelEntity* pEntity = dynamic_cast<CBaseModelEntity*>(g_pGameEntitySystem->GetEntityInstance(CEntityHandle((uint32)entityHandle)));
	if (!pEntity)
	{
...
extern "C" PLUGIN_API void SetEntityOwner(int entityHandle, int ownerHandle)
{
	...
	CBaseEntity* pOwner = dynamic_cast<CBaseEntity*>(g_pGameEntitySystem->GetEntityInstance(CEntityHandle((uint32)ownerHandle)));
	if (!pOwner)
	{
		g_Logger.LogFormat(LS_WARNING, "Cannot 'SetEntityOwner' on invalid entity handle: %d\n", entityHandle);
		return;
...
extern "C" PLUGIN_API void SetEntityParent(int entityHandle, int parentHandle)
{
	...
	CBaseEntity* pNewParent = dynamic_cast<CBaseEntity*>(g_pGameEntitySystem->GetEntityInstance(CEntityHandle((uint32)parentHandle)));
	if (!pNewParent)
	{
		g_Logger.LogFormat(LS_WARNING, "Cannot 'SetEntityParent' on invalid entity handle: %d\n", entityHandle);
		return;
